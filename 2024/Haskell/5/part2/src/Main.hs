...
validUpdate m u = if isUpdateValid m u then Nothing else Just $ middle $ reorder m u
...
reorder :: MultiMap Int Int -> [Int] -> [Int]
reorder m = sortBy (\t1 t2 -> if t2 `elem` mLookup t1 m then LT else GT) 
